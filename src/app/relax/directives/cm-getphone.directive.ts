...
export class CmGetphoneDirective {

  @Input('CmGetphone') _id: string;
  ...
  @HostListener('click') onClick() {
    if (!this.phoneNumber && !this._requestLoading) {
      ...
      this.http.post('/common/lookParentTelphone', { paramJson: JSON.stringify({ id: this._id.toString() }) }).then(res => {
        if (res.code == 1000) {
